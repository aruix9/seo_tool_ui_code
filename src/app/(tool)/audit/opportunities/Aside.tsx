      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
        ...
        <div className="space-y-4">
          {Object.values(gaps).map((url, i) => (
            <div
              key={i}
              className="p-4 rounded-xl border border-slate-200 bg-primary/5"
            >
              <div className="flex justify-between mb-2">
                <p className="text-sm">
                  ...
                  <br />
                  {i === 0 && (
                    <small className="uppercase text-slate-400">master</small>
                  )}
                </p>
                <span className="text-xs font-black self-end text-primary leading-5">
                  {url.missingPercentage}%
                </span>
              </div>
              <div className="h-1.5 w-full bg-primary/5 border border-primary/10 rounded-full">
                <div
                  className="h-full bg-primary rounded-full"
                  style={{ width: `${url.missingPercentage}%` }}
                ></div>
              </div>
            </div>
          ))}
