...
const HeroDomainCards = ({mainUrl, allUrls, auditData}: {mainUrl: string; allUrls: string[]}) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {allUrls.map((url, i) => (
                <div
                    ...
                    {(url === mainUrl) && <div className="absolute top-0 right-0 p-2">
                        <span className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Current</span>
                    </div>}
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">{(url === mainUrl) ? `Your Domain` : `Competitor ${i}`}</p>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <div>
                            ...
                            <p className="text-lg font-bold text-primary">{auditData[url].domain_inlink_rank}</p>
                        </div>
                        <div>
                            <p className="text-[10px] text-slate-400 font-medium">Ref Domains</p>
                            <p className="text-lg font-bold">{formatNumber(auditData[url].dofollow_refdomains)}</p>
                        </div>
                        <div>
                            <p className="text-[10px] text-slate-400 font-medium">Backlinks</p>
                            <p className="text-lg font-bold">{formatNumber(auditData[url].backlinks)}</p>
                        </div>
                    </div>
                </div>
            ))}
...
export default HeroDomainCards
