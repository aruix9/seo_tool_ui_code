...
const RefDomainBacklinkProgress = ({ mainUrl, allUrls, auditData }: { mainUrl: string; allUrls: string[] }) => {
    ...
    return (
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                ...
                {Object.keys(data).map((url, i) =>
                    <div key={i} className="space-y-2">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider mb-1">
                            <span className={`${url === mainUrl ? 'text-primary' : 'text-slate-500'}`}>{url}&nbsp;{i === 0 ? "(leader)" : url === mainUrl ? "(Your Domain)" : i >= 1 && `(Competitor ${i})`}</span>
                            <span className="text-slate-900">{Number(data[url].dofollow_refdomains).toLocaleString('en-IN')}</span>
                        </div>
                        <div className="h-4 w-full bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-1000 ${url === mainUrl ? 'bg-primary' : i > 1 ? 'bg-slate-200' : 'bg-slate-900'}`} style={{ width: `${progressWidth(data[url].dofollow_refdomains)}%` }}></div>
                        </div>
                    </div>
                )}
            </div>
                ...
                <div className="flex flex-wrap">
                    {allUrls.map((url, i) =>
                        ...
                        <div className='inline-flex gap-2 items-center mr-4' key={i}>
                            <span className="text-slate-600 font-bold lowercase tracking-tighter text-[10px]">{url} Gap</span>
                            <div className={`flex items-center gap-1 text-sm ${checkGap(url, mainUrl, data)[1] ? 'text-blue-500' : 'text-red-400'}`}>
                                {checkGap(url, mainUrl, data)[1] ? <ArrowUp className='size-4' /> : <ArrowDown className='size-4' />}
                                <span>{Number(checkGap(url, mainUrl, data)[0]).toLocaleString('en-IN')}%</span>
                            </div>
                        </div> 
                    )}
                </div>
            ...
            <div className="mt-4">
                <Link
                    href={`/audit/compare/refdomains?targets=${allUrls.join(',')}`}
