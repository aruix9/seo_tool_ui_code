...
const ModalAiBrief = ({
  ...
  return (
    <div
      className={`modal-overlay fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm items-center justify-center p-4 ${isModalOpen ? "active" : ""}`}
      id="ai-modal"
    >
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div>
            <ReactMarkdown remarkPlugins={[remarkGfm]}>
              {modalContent.text.replaceAll("*", "\n-")}
            </ReactMarkdown>
          </div>
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-3">
              Referenced Sources ({modalContent.links.length})
            </h4>
            {modalContent.links.map((link, i) => (
              <div key={i} className="grid gap-3 mb-4">
                <Link
                  className="flex items-start gap-3 p-3 rounded-lg border border-primary/10 bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors group"
                  href={link}
                >
