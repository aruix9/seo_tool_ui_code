...
// get Summary
export async function getAllRefDomains() {
  const response = await axios.get(API_BASE + "/api/v1/audit/all-refdomains");
  return response.data;
}

export async function getAllAnchors() {
  const response = await axios.get(API_BASE + "/api/v1/audit/all-anchors");
  return response.data;
}
